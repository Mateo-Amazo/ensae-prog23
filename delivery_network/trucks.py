...
def utiliteprix_write(x,y,data_path = "/home/onyxia/ensae-prog23-1/input/"):
    ...
    n = len(liste_profits)
    liste_camions = sorted(recup_camions(x, data_path),key = lambda x : x[0]/x[1], reverse=True)
    res = []
    for val in liste_camions:
        power,prix = val
        max_profit = 0
        ind = 0
        i = 0
        booleen = False    
        while i < n:
            val2 = liste_profits[i]
            profit,power_min,z = val2
            if power_min <= power and profit > max_profit and z != 1:
                max_profit = profit
                ind = i
                booleen = True
            i += 1
        if booleen :
            liste_profits[ind][-1] = 1
            res += [(prix, profit, ind, profit/prix)]
